...
-define(ascii_hex_digit(L),
    (?ascii_digit(L) orelse
        (L >= $A andalso L =< $F) orelse
        (L >= $a andalso L =< $f))
).
-define(ws(S), (S =:= $\s orelse S =:= $\t orelse S =:= $\n orelse S =:= $\f)).
...
string(Bin, Options) ->
    M1 =
        case proplists:get_value(event_fun, Options) of
    ...
    M2 =
        case proplists:get_value(user_state, Options) of
            undefined ->
                M1;
            V ->
...
combine([], _) -> "";
combine([Last], _) -> [Last];
combine([Head | Tail], Space) -> [Head, Space, combine(Tail, Space)].
...
tag_name_1(Stream, Pos) ->
    case Stream of
        <<_:Pos/binary, C1, C2, C3, _/binary>> when
            ?lower_ascii_letter(C1),
            ?lower_ascii_letter(C2),
            ?lower_ascii_letter(C3)
        ->
            tag_name_1(Stream, Pos + 3);
        <<_:Pos/binary, C1, C2, _/binary>> when
            ?lower_ascii_letter(C1),
            ?lower_ascii_letter(C2)
        ->
...
rcdata_end_tag_name(
    ...
    #{
        ...
        last_start_tag := #start_tag{name = Name}
    } = State
) ->
    before_attribute_name(Rest, ?i(State));
rcdata_end_tag_name(
...
script_data_double_escape_start(<<$\n, Rest/binary>>, State) ->
    State1 = emit(#char{data = $\n}, ?i(State)),
    script_data_escaped(Rest, State1);
script_data_double_escape_start(<<C, Rest/binary>>, #{temp_buffer := "tpircs"} = State) when
    ?ws(C);
    C =:= $/;
    C =:= $>
->
    script_data_double_escaped(Rest, State);
script_data_double_escape_start(<<C, Rest/binary>>, State) when
    ?ws(C);
    C =:= $/;
    C =:= $>
->
    State1 = emit(#char{data = C}, State),
    script_data_escaped(Rest, State1);
script_data_double_escape_start(<<C, Rest/binary>>, #{temp_buffer := Buff} = State) when
    ?upper_ascii_letter(C)
->
    State1 = emit(#char{data = C}, State),
...
script_data_double_escape_end(<<$\n, Rest/binary>>, State) ->
    State1 = emit(#char{data = $\n}, ?i(State)),
    script_data_double_escaped(Rest, State1);
script_data_double_escape_end(<<C, Rest/binary>>, #{temp_buffer := "tpircs"} = State) when
    ?ws(C);
    C =:= $/;
    C =:= $>
->
    script_data_escaped(Rest, State);
script_data_double_escape_end(<<C, Rest/binary>>, State) when
    ?ws(C);
    C =:= $/;
    C =:= $>
->
    State1 = emit(#char{data = C}, State),
    script_data_double_escaped(Rest, State1);
script_data_double_escape_end(<<C, Rest/binary>>, #{temp_buffer := Buff} = State) when
    ?upper_ascii_letter(C)
->
    State1 = emit(#char{data = C}, State),
    State2 = State1#{temp_buffer := [C + 32 | Buff]},
...
before_attribute_name(<<C, _/binary>> = Stream, State0) when
    C == $/;
    C == $>
->
...
attribute_name(<<C, _/binary>> = Stream, State) when
    ?ws(C);
    C == $/;
    C == $>
->
...
attribute_name(
    ...
    #{
        current_token :=
            #start_tag{attributes = [At | Ats]} = Curr
    } = State
) ->
...
attribute_name_1(<<C, _/binary>> = Stream, Acc) when
    ?ws(C);
    C == $/;
    C == $>;
    C == $=
...
attribute_value_unquoted(<<C, Rest/binary>>, #{current_token := Curr} = State) when
    C =:= $\";
    C =:= $';
    C =:= $<;
    C =:= $=;
...
markup_declaration_open(<<D, O, C, T, Y, P, E, Rest/binary>>, State) when
    D =:= $d orelse D =:= $D,
    O =:= $o orelse O =:= $O,
    C =:= $c orelse C =:= $C,
    T =:= $t orelse T =:= $T,
...
after_doctype_name(<<$>, Rest/binary>>, #{current_token := Curr} = State) ->
    State1 = emit(Curr, State),
    data(Rest, State1);
after_doctype_name(<<P, U, B, L, I, C, Rest/binary>>, State) when
    P =:= $p orelse P =:= $P,
    U =:= $u orelse U =:= $U,
    B =:= $b orelse B =:= $B,
    L =:= $l orelse L =:= $L,
...
after_doctype_name(<<S, Y, Z, T, E, M, Rest/binary>>, State) when
    S =:= $s orelse S =:= $S,
    Y =:= $y orelse Y =:= $Y,
    Z =:= $s orelse Z =:= $S,
    T =:= $t orelse T =:= $T,
...
character_reference(<<C, _/binary>> = Stream, State) when
    ?ws(C);
    C =:= $<;
    C =:= $&
->
...
%% 8.2.4.73
numeric_character_reference(<<C, Rest/binary>>, #{temp_buffer := Buff} = State0) when
    C =:= $x;
    C =:= $X
->
    hexadecimal_character_reference_start(Rest, State0#{
        temp_buffer := [C | Buff],
        char_ref_code := 0
    });
...
%% 8.2.4.74
hexadecimal_character_reference_start(<<C, _/binary>> = Stream, State0) when
    ?ascii_hex_digit(C)
->
...
%% 8.2.4.75
decimal_character_reference_start(<<C, _/binary>> = Stream, State0) when
    ?ascii_digit(C)
->
...
%% 8.2.4.76
hexadecimal_character_reference(<<C1, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1)
->
    Code1 = list_to_integer([C1], 16),
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1), ?ascii_hex_digit(C2)
->
    Code1 = list_to_integer([C1, C2], 16),
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, C3, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1),
    ?ascii_hex_digit(C2),
    ?ascii_hex_digit(C3)
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, C3, C4, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1),
    ?ascii_hex_digit(C2),
    ?ascii_hex_digit(C3),
    ?ascii_hex_digit(C4)
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, C3, C4, C5, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1),
    ?ascii_hex_digit(C2),
    ?ascii_hex_digit(C3),
    ?ascii_hex_digit(C4),
...
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, C3, C4, C5, C6, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1),
    ?ascii_hex_digit(C2),
    ?ascii_hex_digit(C3),
    ?ascii_hex_digit(C4),
...
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
hexadecimal_character_reference(<<C1, C2, C3, C4, C5, C6, C7, $;, Rest/binary>>, State0) when
    ?ascii_hex_digit(C1),
    ?ascii_hex_digit(C2),
    ?ascii_hex_digit(C3),
    ?ascii_hex_digit(C4),
...
%% 8.2.4.77
decimal_character_reference(<<C1, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1)
->
    Code1 = list_to_integer([C1]),
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1), ?ascii_digit(C2)
->
    Code1 = list_to_integer([C1, C2]),
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1),
    ?ascii_digit(C2),
    ?ascii_digit(C3)
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, C4, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1),
    ?ascii_digit(C2),
    ?ascii_digit(C3),
    ?ascii_digit(C4)
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, C4, C5, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1),
    ?ascii_digit(C2),
    ?ascii_digit(C3),
    ?ascii_digit(C4),
...
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, C4, C5, C6, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1),
    ?ascii_digit(C2),
    ?ascii_digit(C3),
    ?ascii_digit(C4),
...
->
    ...
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, C4, C5, C6, C7, $;, Rest/binary>>, State0) when
    ?ascii_digit(C1),
    ?ascii_digit(C2),
    ?ascii_digit(C3),
    ?ascii_digit(C4),
...
dispatch(#{insertion_mode := in_head} = State, Token) ->
    case Token of
        ...
        ->
            State1 = maybe_pop_text(State),
            % should be "head"
            State2 = pop_all_to_tag(Name, State1),
...
dispatch(#{insertion_mode := in_body} = State, Token) ->
    case Token of
        ...
        ->
            State1 = maybe_pop_text(State),
            State2 =
                case get_current_node(State1) of
                    #start_tag{name = N1} when
                        N1 == <<"h1">>;
                        N1 == <<"h2">>;
                        N1 == <<"h3">>;
                        N1 == <<"h4">>;
...
%% in_frameset state
dispatch(#{insertion_mode := in_frameset} = State, _Token) ->
    State;
%% after_frameset state
dispatch(#{insertion_mode := after_frameset} = State, _Token) ->
    State;
...
norm_tok(
    ...
    Closing1 =
        (Closing orelse Name == <<"area">> orelse Name == <<"base">> orelse
...
generate_implied_end_tags(#{open_elements := [#start_tag{name = Nm} | Els]} = State) when
    Nm == <<"dd">>;
    Nm == <<"dt">>;
    Nm == <<"li">>;
    Nm == <<"optgroup">>;
...
generate_implied_end_tags(#{open_elements := [#start_tag{name = Nm} | Els]} = State, Except) when
    Nm =/= Except andalso
...
add_math_element(
    ...
    case Closing of
        false ->
            add_open_element(Token#start_tag{name = {mathml, N}}, State2#{
                inscope_namespace := [mathml | Ns]
            });
        true ->
...
add_svg_element(
    ...
    case Closing of
        false ->
            add_open_element(Token#start_tag{name = {svg, N}}, State2#{
                inscope_namespace := [svg | Ns]
            });
        true ->
